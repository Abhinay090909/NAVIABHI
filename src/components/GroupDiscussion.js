...
function GroupDiscussion({ policies, onVoteComplete }) {
  ...
  const handleTalkToAgent = async (agent, userMessage) => {
    ...
    const reply = await getAgentGPTResponse(agent, category, userMessage);
    setTypingAgent(null);
  ...
  const renderDesk = (agent) => {
    ...
    return (
      <div key={agent.agentId} style={deskCard}>
        ...
        <button onClick={() => speakText(justification)} style={smallButton}>
          🎧 Hear Justification
        </button>
        <button onClick={() => handleVoiceQuestion(agent)} style={{ ...smallButton, backgroundColor: "#6c5ce7" }}>
          🎙️ Speak to {agent.name}
        </button>
        ...
        <div style={{ marginTop: "8px" }}>
          ...
          />
          <button onClick={() => handleManualSubmit(agent)} style={{ ...smallButton, width: "100%" }}>
            📩 Send
          </button>
          <button onClick={() => clearAgentChat(agent.agentId)} style={{ ...smallButton, marginTop: "4px", backgroundColor: "#d63031" }}>
            🔄 Clear
          </button>
        </div>
  ...
  return (
    <div style={container}>
      ...
      <div style={instruction}>
        ...
        [2] Moderate Change (your choice) &nbsp;&nbsp;
        [3] Full Transformation
      </div>

      <div style={transcriptHint}>
        {userTranscript || "🗣️ Ask a question by speaking or typing below."}
      </div>

      <div style={gridRow}>
        {agents.slice(0, 3).map(renderDesk)}
      </div>
      ...
      <div style={{ textAlign: "center" }}>
        {currentCategoryIndex > 0 && (
          <button onClick={() => setCurrentCategoryIndex(currentCategoryIndex - 1)} style={navButton("#636e72")}>⬅ Back</button>
        )}
